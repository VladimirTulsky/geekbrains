...
public class Controller implements Initializable {
    ...
    static Map<String, String> censored = new HashMap<>();
    static {
        censored.put("владимир", "Владимир");
    }

    //метод проверки на цензуру
    private static String censoredMsg(String str) {
        return censored.getOrDefault(str, str);
    }
        ...
        try {
            ...
            new Thread(() -> {
                try {
                    ...
                    showMsg(nickname, textArea);
                    //продолжение цикла работы
                    while (true) {
                        String str = in.readUTF();
                        if (str.startsWith("/")) {
                            if (str.equals(Command.END)) {
    ...
    public void sendMsg(ActionEvent actionEvent) {
        try {
            if (textField.getText().trim().length() > 0) {
                //проверка на цензуру
                String str = censoredMsg(textField.getText());
                out.writeUTF(str);
                textField.clear();
    ...
    //создание файла
    private static String prepareFileName(String login) throws IOException {
        File file = new File("C:\\Users\\beyon\\textfiles\\" + login + ".txt");
        file.createNewFile();
    ...
    private static void writeMsgIntoFile(String nickname, String str) throws IOException {
        String filePath = prepareFileName(nickname);
        try {
    ...
    //показать сообщения из файла
    private static void showMsg(String nickname, TextArea textArea) throws IOException {
        String filePath = prepareFileName(nickname);
        if(!filePath.isEmpty()) {
